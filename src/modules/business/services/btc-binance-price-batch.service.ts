...
import { BtcBinancePriceRepository, BatchInsertBinancePrice } from '@/database/repositories/btc-binance-price.repository';
import { PRICE_BATCH_INTERVAL, PRICE_BATCH_SIZE, MAX_QUEUE_SIZE } from '@/shared/constants/polymarket.constants';

@Injectable()
export class BtcBinancePriceBatchService implements OnModuleDestroy {
  private priceQueue: BatchInsertBinancePrice[] = [];
  private batchTimer: NodeJS.Timeout | null = null;
  private isProcessing: boolean = false;

  constructor(
    private readonly priceRepository: BtcBinancePriceRepository,
    @InjectPinoLogger(BtcBinancePriceBatchService.name) private readonly logger: PinoLogger,
  ) {}
  ...
  async savePrice(timestamp: Date, symbol: string, price: number): Promise<void> {
    ...
    if (this.priceQueue.length >= MAX_QUEUE_SIZE) {
      this.logger.error(
        ...
        '🔴 [BtcBinancePriceBatchService] [savePrice] Queue overflow, dropping price',
      );
      return;
    }
    ...
    if (this.priceQueue.length >= PRICE_BATCH_SIZE) {
      this.logger.debug(
        { queueSize: this.priceQueue.length },
        '🔄 [BtcBinancePriceBatchService] [savePrice] Queue reached BATCH_SIZE, processing immediately',
      );
    ...
    try {
      ...
      if (pricesToProcess.length > 0) {
        ...
        this.logger.debug(
          ...
          `✅ [BtcBinancePriceBatchService] [processBatch] Inserted ${pricesToProcess.length} prices`,
        );
      }
    } catch (error) {
      this.logger.error(
        { error: error instanceof Error ? error.message : String(error), count: this.priceQueue.length },
        '🔴 [BtcBinancePriceBatchService] [processBatch] Error inserting prices',
      );
  ...
  async flushQueue(): Promise<void> {
    ...
    if (this.priceQueue.length > 0) {
      this.logger.info(
        ...
        '🔄 [BtcBinancePriceBatchService] [flushQueue] Flushing remaining prices in queue',
      );
  ...
  async onModuleDestroy(): Promise<void> {
    this.logger.info('🔄 [BtcBinancePriceBatchService] [onModuleDestroy] Cleaning up');
